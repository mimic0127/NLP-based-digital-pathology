...
parser.add_argument('--TCGA_folder_path2',type=str, default="/data/cm/MSI_new/CRC_patch/TCGA_pic_BRAFadded", help='Second Patch path for test')
...
if __name__ == '__main__':
    ...
    id_msi.columns = ['ID','CIMP'] 
    id0 = id_msi[id_msi['CIMP']==0]['ID'].values.tolist()
    id1 = id_msi[id_msi['CIMP']==1]['ID'].values.tolist()
    ...
    #ID_path_0 = ID_path_0[:12]
    #ID_path_1 = ID_path_1[:12]
    index_0 = range(len(ID_path_0))
    index_1 = range(len(ID_path_1))

    test_id = np.load('/data/cm/NLP_based_model_code/CIMP_id.npy').tolist()
    test_feature_paths = []
    n = 0
    for i in test_id:
        if i[-1] != '2':
    ...
    if args.model_save_path == None:
        model_save_path = args.output_dir + '/CIMP_' + model_name + '.pth'
    else:
