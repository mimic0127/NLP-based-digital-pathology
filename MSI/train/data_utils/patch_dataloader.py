...
class MSIDataset(data.Dataset):
    def __init__(self, file_path = [], MCO = True, train = True, batch_size = 32):
        self.imgs_path = file_path
        self.MCO = MCO         
        self.train = train
        self.batch_size = batch_size
        if self.MCO:
            id_msi_df = pd.read_csv('/data/cm/NLP_based_model_code/Ground_Truth/MCO_MSI.csv')
            self.id_msi = id_msi_df[['ParentSpecimen','msi_status']]
            self.id_msi.columns = ['ID','MSI'] 
            # 'MSS','MSI'
        if not self.MCO:
            id_msi_df = pd.read_csv('/data/cm/NLP_based_model_code/Ground_Truth/Exp5_TEST_RESULT_PATIENT_SCORES_FULL.csv')
            self.id_msi = id_msi_df[['patients','y_true_label']]
            self.id_msi.columns = ['ID','MSI']
            # 'nonMSIH','MSIH'
    ...
    def __getitem__(self, index):
        ...
        plabel = self.id_msi[self.id_msi['ID']==pname]['MSI'].values
        if plabel == 'MSS' or plabel == 'nonMSIH':
            label = 0
        elif plabel == 'MSI' or plabel == 'MSIH':
            label = 1
        else:
            print('error')
...
class MSIDataset_inter(data.Dataset):
...
class MSIDataset_inter_256(data.Dataset):
    def __init__(self, file_path = [], MCO = True, train = True, batch_size = 64):
        self.imgs_path = file_path
        self.MCO = MCO         
        self.train = train
