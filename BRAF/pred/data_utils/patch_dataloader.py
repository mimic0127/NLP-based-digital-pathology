...
class MSIDataset(data.Dataset):
    def __init__(self, file_path = [], MCO = True, train = True, batch_size = 64):
        ...
        if self.MCO:
            id_msi_df = pd.read_csv('/data/cm/NLP_based_model_code/Ground_Truth/BRAF_label.csv')
            self.id_msi = id_msi_df[['ID','label']]
            self.id_msi.columns = ['ID','BRAF'] 
            # 'MSS','MSI'
        if not self.MCO:
            id_msi_df = pd.read_csv('/data/cm/NLP_based_model_code/Ground_Truth/TCGA_labels.csv')
            self.id_msi = id_msi_df[['ID','BRAF_mutation']]
            self.id_msi.columns = ['ID','BRAF']
            # 'nonMSIH','MSIH'
    ...
    def __getitem__(self, index):
        ...
        plabel = self.id_msi[self.id_msi['ID']==pname]['BRAF'].values
        if plabel == 0 or plabel == 'WT':
            label = 0
        elif plabel == 1 or plabel == 'MUT':
            label = 1
        else:
            print('error')
