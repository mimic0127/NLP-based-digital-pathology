...
def predict_patient_level(patient_path, model, device, MCO = True):
    ...
    dataset = MSIDataset(patch_path_ls,MCO = MCO,train = False)
...
parser.add_argument('--TCGA_folder_path',type=str, default="/data/cm/MSI_new/CRC_patch/TCGA_pic", help='TCGA tile path')
parser.add_argument('--TCGA_folder_path2',type=str, default="/data/cm/MSI_new/CRC_patch/TCGA_pic_BRAFadded", help='TCGA tile path2')
parser.add_argument('--model_name', type=str, default='Sequencer2D')
parser.add_argument('--model_save_path', type=str,default = '/data/cm/MSI_new/models/BRAF_Sequencer2D.pth')
    ...
    for i in test_id:
        if i[-1] != '2':
            if os.path.exists(test_folder_path + '/' + i[:12]):
                strpath = [test_folder_path + '/' + i[:12]]
                test_feature_paths = test_feature_paths + strpath
                n = n + 1
            elif os.path.exists(test_folder_path2 + '/' + i[:12]):
                strpath = [test_folder_path2 + '/' + i[:12]]
                test_feature_paths = test_feature_paths + strpath
                n = n + 1
            else:
                print(i)

    test_paths = test_feature_paths
    model_name = 'BRAF_' + args.model_name
