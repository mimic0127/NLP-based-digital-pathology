...
from timm.models import swin_small_patch4_window7_224, vit_base_patch16_224
...
parser.add_argument('--TCGA_folder_path',type=str, default="/data/cm/MSI_new/CRC_patch/TCGA_pic_mix", help='Patch path for test')
parser.add_argument('--TCGA_folder_path2',type=str, default="/data/cm/MSI_new/CRC_patch/TCGA_pic_BRAFadded", help='TCGA feature path2')
parser.add_argument('--MCO_MSS_path',type=str, default="/data/cm/MSI_new/CRC_patch/MCO_pic_0",help='patch path for train with status-0')
parser.add_argument('--MCO_MSI_path',type=str, default="/data/cm/MSI_new/CRC_patch/MCO_pic_1",help='patch path for train with status-1')
parser.add_argument('--model_name',type=str, default='Sequencer2D', help='model name')
...
def train(model_name, train_path_list, val_path_list, test_path_list, model_save_path, num_epochs, lr, device):
    ...
    :param device: gpu device
    """
    train_data = MSIDataset(train_path_list, train=True,MCO=True)
    train_loader = train_data.get_loader()

    val_data = MSIDataset(val_path_list, train=False,MCO=True)
    val_loader = val_data.get_loader()

    test_data = MSIDataset(test_path_list, train=False,MCO=False)
    test_loader = test_data.get_loader()
 
    # model definition
    if model_name == 'ResNet18':
        model = models.resnet18(pretrained=False)
        model.load_state_dict(torch.load('/data/cm/NLP_based_model_code/pretrained_model/resnet18-5c106cde.pth'))
        num_ftrs = model.fc.in_features
        model.fc = nn.Linear(num_ftrs,2)
        model = model.to(device)
    elif model_name == 'ResNet50':
        model = models.resnet50(pretrained=False)
        model.load_state_dict(torch.load('/data/cm/NLP_based_model_code/pretrained_model/resnet50-19c8e357.pth'))
        num_ftrs = model.fc.in_features
        model.fc = nn.Linear(num_ftrs,2)
        model = model.to(device)
        ...
        weighted_path = "/data/cm/pretrained_model/NLP_based_model_code/efficientnet-b0-355c32eb.pth"
        model = EfficientNet.from_pretrained('efficientnet-b0',weights_path=weighted_path, num_classes=2)
        model = model.to(device)
    elif model_name == 'ViT':
        model = vit_base_patch16_224(pretrained=False)
        model.load_state_dict(torch.load('/data/cm/NLP_based_model_code/pretrained_model/jx_vit_base_p16_224-80ecf9dd.pth'))
        ...
        num_ftrs = model.head.in_features
        model.head = nn.Linear(num_ftrs, 21841)
        model.load_state_dict(torch.load('/data/cm/NLP_based_model_code/pretrained_model/swin_small_patch4_window7_224_22k.pth')['model'])
        num_ftrs = model.head.in_features
        model.head = nn.Linear(num_ftrs, 2)
        model.to(device)
    elif model_name == 'CMT':
    ...
    criterion = torch.nn.CrossEntropyLoss(weight = torch.tensor([0.4,1-0.4]).float().to(device))
    criterion_val = torch.nn.CrossEntropyLoss(weight = torch.tensor([0.4,1-0.4]).float().to(device))  # 验证用的loss
...
if __name__ == '__main__':
    ...
    test_folder_path2 = args.TCGA_folder_path2
    ...
    id_msi_df = pd.read_csv('/data/cm/MSI_new/BRAF_label.csv')
    id_msi = id_msi_df[['ID','label']]
    id_msi.columns = ['ID','BRAF'] 
    id0 = id_msi[id_msi['BRAF']==0]['ID'].values.tolist()
    id1 = id_msi[id_msi['BRAF']==1]['ID'].values.tolist()

    # depend on patient
    ID_path_MSS0 = glob.glob(MCO_MSS_path+'/*')
    ID_path_MSI1 = glob.glob(MCO_MSI_path + '/*')
    ID_path_0 = []
    ID_path_1 = []
    for i in id0:
        if MCO_MSS_path + '/' + i in ID_path_MSS0:
            ID_path_0.append(MCO_MSS_path + '/' + i)
        elif MCO_MSI_path + '/' + i in ID_path_MSI1:
            ID_path_0.append(MCO_MSI_path + '/' + i)
    for i in id1:
        if MCO_MSS_path + '/' + i in ID_path_MSS0:
            ID_path_1.append(MCO_MSS_path + '/' + i)
        elif MCO_MSI_path + '/' + i in ID_path_MSI1:
            ID_path_1.append(MCO_MSI_path + '/' + i)

    index_0 = range(len(ID_path_0))
    index_1 = range(len(ID_path_1))


    test_id = np.load('/data/cm/NLP_based_model_code/BRAF_id.npy').tolist()
    test_feature_paths = []
    n = 0
    for i in test_id:
        if i[-1] != '2':
            if os.path.exists(test_folder_path + '/' + i[:12]):
                test_feature_paths = test_feature_paths + glob.glob(test_folder_path + '/' + i[:12] + '/*')
                n = n + 1
            elif os.path.exists(test_folder_path2 + '/' + i[:12]):
                test_feature_paths = test_feature_paths + glob.glob(test_folder_path2 + '/' + i[:12] + '/*')
                n = n + 1
            else:
                print(i)
    ...
    if args.model_save_path == None:
        model_save_path = args.output_dir + '/BRAF_' + model_name + '.pth'
    else:
